...
在这个过程中，知名品牌起到了行业推动型的作用，如有道、阿尔法蛋，推出自身的千元级旗舰机型扫描笔产品，用其出色的内容质量和产品体验，在市场上获得大量的用户认可，一时间，传统教育品牌、教培机构、互联网大厂也纷纷入局，行业中的产品和方案百花争鸣。
...
|AI算法|	提供扫描笔所需算法：OCR、翻译、TTS，集合高算力芯片发挥最优效果；|	需原厂配合集成，调优、迭代时间长；|
|教育内容	|正版版权词典、聆思核心教育算法（国家考试指定语音评测算法、英文作文批改等）；|	非正版内容，产品教育能力弱；|
...
| 离线能力	 | CSK：离线裁剪、离线拼接；上位机：连接网络、驱屏	| CSK：离线裁剪、离线拼接、离线OCR；上位机：离线翻译、离线TTS、离线查词；上位机：连接网络、驱屏|	CSK：离线裁剪、离线拼接、离线OCR、离线翻译、离线TTS、离线查词；上位机：连接网络、驱屏|
| 方案特点|	相比纯WIFI方案，低宽带情况下（30Kbps）依旧可用；|	CSK4002可搭配任意上位机（安卓/linux），形成全套扫描笔离在线方案；|市面唯一离线跑全功能的离在线RTOS方案|
...
|离线能力|	CSK：离线裁剪、离线拼接、离线OCR；蓝牙：连接小程序/APP|	CSK：离线裁剪、离线拼接、离线OCR、离线翻译、离线TTS、离线查词；蓝牙：连接小程序/APP |
|方案特点|	低成本RTOS外设方案，产品设计上不可带屏幕；|低成本RTOS主控方案，一颗芯片可完成离线AI、业务逻辑和硬件外设驱动；|
...
##### 离线OCR
介绍：
基于聆思自研深度神经网络模型的端到端文字识别系统，将扫描到课本教材和儿童读物的内容，直接转换为文本，实现高精准、毫秒级识别体验。
...
##### 离线语音合成
介绍：
将文字信息转化为声音信息，同时提供了众多具有特色的音库供产品选择发音人。
...
#### 方案介绍
配置：CSK6011A+XR819s；
特性：低成本RTOS离在线方案（离线OCR+离线TTS+离线翻译+离线查词）；
...
##### 离线语音合成
介绍：
将文字信息转化为声音信息，同时提供了众多具备特色的音库供产品选择发音人。
...
基于聆思自研的行业先进的光学字符识别技术，将扫描到课本教材和儿童读物的内容，直接转换为文本，实现高精准、毫秒级识别体验。
...
介绍：
将文字信息转化为声音信息，同时提供了众多具备特色的音库供产品选择发音人；在2008年英文合成自然度已超过人类平均水平。
